...
use trees_canopy, only:trees_forcing
#endif
!=======================================added by Xiantao Fan
...
use param, only : read_inflow_file  !add by Mingwei for inflow
use sim_param, only : u, v, w
use messages, only : error
use fringe_util
use io, only: inflow_read  !add by Mingwei 
implicit none
...
!add by Mingwei for read inflow
...
if (read_inflow_file) then  !--read vel inflow @ jx = iend_w from file
    u(i_w, 1:ny, 1:nz) = alpha * u(istart, 1:ny, 1:nz) + beta * u(iend_w, 1:ny, 1:nz)
    v(i_w, 1:ny, 1:nz) = alpha * v(istart, 1:ny, 1:nz) + beta * v(iend_w, 1:ny, 1:nz)
    w(i_w, 1:ny, 1:nz) = alpha * w(istart, 1:ny, 1:nz) + beta * w(iend_w, 1:ny, 1:nz)  !need check
else
    u(i_w, 1:ny, 1:nz) = alpha * u(istart, 1:ny, 1:nz) + beta * inflow_velocity
    v(i_w, 1:ny, 1:nz) = alpha * v(istart, 1:ny, 1:nz)
    w(i_w, 1:ny, 1:nz) = alpha * w(istart, 1:ny, 1:nz)
end if
end do
