...
!for inflow write
use io, only :  inflow_write, inflow_read !add by Mingwei
!
use fft
...
time_loop: do jt_step = nstart, nsteps
   ...
   tt = tt+dt
    ...
    !  For trees at the moment all three arrays are assigned.
...
#if defined(PPTREES)
    RHSx(:,:,1:nz-1) = RHSx(:,:,1:nz-1) + fxa_t(:,:,1:nz-1)
    RHSy(:,:,1:nz-1) = RHSy(:,:,1:nz-1) + fya_t(:,:,1:nz-1)
    RHSz(:,:,1:nz-1) = RHSz(:,:,1:nz-1) + fza_t(:,:,1:nz-1)
#endif
...
if (write_inflow_file) call inflow_write () !  Add for inflow write
