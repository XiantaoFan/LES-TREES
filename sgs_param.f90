...
!for the inflow read and write
real(rprec), dimension(:,:), allocatable :: FMM_hold,FLM_hold,FQN_hold,FNN_hold  !add by Mingwei 
...
!for inflow read and write 
use param, only : write_inflow_file, read_inflow_file  !add by Mingwei
...
if (write_inflow_file .or. read_inflow_file) then

allocate ( FLM_hold(ny,lbz:nz) ); FLM_hold = 0.0_rprec
allocate ( FMM_hold(ny,lbz:nz) ); FMM_hold = 0.0_rprec
allocate ( FQN_hold(ny,lbz:nz) ); FQN_hold = 0.0_rprec
allocate ( FNN_hold(ny,lbz:nz) ); FNN_hold = 0.0_rprec
