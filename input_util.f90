...
#ifdef PPTREES
!*******************************************************************************
subroutine trees_block()
!*******************************************************************************
use trees_canopy
implicit none

character(*), parameter :: block_name = 'TREES'
...
do
    ...
    if (block_exit_pos == 0) then
        ...
        select case (uppercase(buff(1:equal_pos-1)))
            case ('NUM_XX')
                read (buff(equal_pos+1:), *) num_xx
            case ('NUM_YY')
                read (buff(equal_pos+1:), *) num_yy
            case ('Cd')
                read (buff(equal_pos+1:), *) Cd
            case ('h_ALL')
                read (buff(equal_pos+1:), *) h_all
            case ('zm_ALL')
                read (buff(equal_pos+1:), *) zm_all
            case ('ORIENTATION1')
                read (buff(equal_pos+1:), *) orientation1
            case ('STAG_PERC1')
                read (buff(equal_pos+1:), *) stag_perc1
            case ('am_ALL')
                read (buff(equal_pos+1:), *) am_all
            case ('Rt_ALL')
                read (buff(equal_pos+1:), *) Rt_all
            case ('Lt_ALL')
                read (buff(equal_pos+1:), *) Lt_all
            case ('p_x')
                read (buff(equal_pos+1:), *) p_x
            case ('p_y')
                read (buff(equal_pos+1:), *) p_y
            case ('p_z')
                read (buff(equal_pos+1:), *) p_z
            case ('READ_PARAM1')
                read (buff(equal_pos+1:), *) read_param1
            case ('T_AVG_DIM1')
                read (buff(equal_pos+1:), *) T_avg_dim1
            case ('ALPHA1')
                read (buff(equal_pos+1:), *) alpha1
            case ('FILTER_CUTOFF1')
                read (buff(equal_pos+1:), *) filter_cutoff1
            case ('TBASE1')
                read (buff(equal_pos+1:), *) tbase1
            case default
                if (coord == 0) write(*,*) 'Found unused data value in '       &
                    // block_name // ' block: ' // buff(1:equal_pos-1)
        end select
