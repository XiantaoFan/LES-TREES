...
real(rprec), dimension(:,:,:), allocatable :: u, v, w,                         &
    ...
    fx, fy, fz, fxa, fya, fza, fxa_t, fya_t, fza_t
real(rprec), target, dimension(:,:,:), allocatable :: p
real(rprec), dimension(:,:,:), allocatable :: u_f, v_f, w_f  ! add by Mingwei
...
!add by Mingwei
allocate ( u_f(ld, ny, lbz:nz) ); u_f = 0.0_rprec
allocate ( v_f(ld, ny, lbz:nz) ); v_f = 0.0_rprec
allocate ( w_f(ld, ny, lbz:nz) ); w_f = 0.0_rprec
...
#ifdef PPTREES
allocate ( fxa_t(ld, ny, lbz:nz) ); fxa_t = 0.0_rprec
allocate ( fya_t(ld, ny, lbz:nz) ); fya_t = 0.0_rprec
allocate ( fza_t(ld, ny, lbz:nz) ); fza_t = 0.0_rprec
#endif
