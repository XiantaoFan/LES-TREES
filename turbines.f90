...
public :: turbines_checkpoint  !add by Mingwei for turbine checkpoint
...
if (read_param) then
    ...
    do k = 1, nloc
        wind_farm%turbine(k)%xloc = wind_farm%turbine(k)%xloc 
        wind_farm%turbine(k)%yloc = wind_farm%turbine(k)%yloc 
        wind_farm%turbine(k)%height = wind_farm%turbine(k)%height 
        wind_farm%turbine(k)%dia = wind_farm%turbine(k)%dia 
        wind_farm%turbine(k)%thk = wind_farm%turbine(k)%thk 
    end do
else
